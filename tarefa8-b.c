...
/*
Código para uma simulação de um AGV (Automated Guided Vehicle) 
que se move para frente seguindo uma linha preta desenhada no chão, 
com sensor de colisão e procedimento de segurança.
*/
...
// Sensores Infravermelhos (Seguidor de Linha)
const int PIN_IR_ESQUERDO = A0;
const int PIN_IR_DIREITO = A1;
...
const int FATOR_CURVA = 50; // Percentual da velocidade atual aplicado à roda interna durante a curva (em %)
...
// Movimenta o AGV para a direita:
void virar_a_direita() {
    // Mantém a roda esquerda na velocidade atual e reduz a roda direita (curva para a direita):
    analogWrite(MOTOR_E_EN, velocidadeAtual);
    analogWrite(MOTOR_D_EN, velocidadeAtual * FATOR_CURVA / 100);

    // Ambos os motores seguem para frente (L293D: IN1=HIGH, IN2=LOW):
    digitalWrite(MOTOR_E_IN1, HIGH);
    digitalWrite(MOTOR_E_IN2, LOW);
...
// Movimenta o AGV para a esquerda:
void virar_a_esquerda() {
    // Mantém a roda direita na velocidade atual e reduz a roda esquerda (curva para a esquerda):
    analogWrite(MOTOR_E_EN, velocidadeAtual * FATOR_CURVA / 100);
    analogWrite(MOTOR_D_EN, velocidadeAtual);
...
// Lê o sensor infravermelho esquerdo:
bool lerSensorEsquerdo() {
    return digitalRead(PIN_IR_ESQUERDO) == LOW; // Retorna true se ele estiver sobre a linha preta
}

// Lê o sensor infravermelho direito:
bool lerSensorDireito() {
    return digitalRead(PIN_IR_DIREITO) == LOW; // Retorna true se ele estiver sobre a linha preta
}

// Lê os dois sensores infravermelhos e direciona o AGV para seguir a linha preta:
void seguirLinha() {
    bool linhaEsquerda = lerSensorEsquerdo();
    bool linhaDireita = lerSensorDireito();

    if(!linhaEsquerda && !linhaDireita) { // AGV está sobre a linha preta, segue em frente
        avancar();
    }
    else if(linhaEsquerda && !linhaDireita) { // AGV está desviando para a direita, corrige para a esquerda
        virar_a_esquerda();
    }
    else if(!linhaEsquerda && linhaDireita) { // AGV está desviando para a esquerda, corrige para a direita
        virar_a_direita();
    }
    else {
        parar();
...
void setup() {
    ...
    // Configura pinos dos sensores infravermelhos:
    pinMode(PIN_IR_ESQUERDO, INPUT);
    pinMode(PIN_IR_DIREITO, INPUT);
}
